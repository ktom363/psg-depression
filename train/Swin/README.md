...
```bash
python train_swin.py --image-root /path/to/images \
                     --labels labels.csv \
                     --output-dir runs/swin
```
...
|---|---|---|
| `--model-name` | `swin_base_patch4_window7_224` | timm model identifier. |
...
```
runs/swin/best_swin.pth             lowest-validation-loss checkpoint
runs/swin/training_history.csv      per-epoch loss, accuracy, participant-level metrics, learning rate
runs/swin/split.json                participant identifiers per subset
runs/swin/test_predictions.csv      participant-level probabilities on the internal test set
runs/swin/run_config.json           resolved settings, parameter counts, library versions
```
